...
class Program
{
	const string URL_ELEMENTS = "https://developer.mozilla.org/en-US/docs/Web/HTML/Element";
	const string URL_ATTRIBUTES = "https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes";
	...
	static void ScrapeElements(Dictionary<string, List<string>> htmlElements)
	{
		...
		IDocument document = context.OpenAsync(URL_ELEMENTS).GetAwaiter().GetResult();
	...
	static void ScrapeAttributes(Dictionary<string, List<string>> htmlElements)
	{
		IConfiguration config = Configuration.Default.WithDefaultLoader();
		IBrowsingContext context = BrowsingContext.New(config);
		IDocument document = context.OpenAsync(URL_ATTRIBUTES).GetAwaiter().GetResult();
