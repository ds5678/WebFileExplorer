...
internal class Program
{
	...
	private const string JsonFile = RepositoryRoot + "Scraper/bin/Debug/net8.0/html-elements-attributes.json";
	private const string TargetProject = RepositoryRoot + "HtmlSharp/";
	...
	static void Main()
	{
		...
		foreach ((string element, List<string> attributes) in elements)
		{
			...
			writer.WriteLineNoTabs();
			writer.WriteFileScopedNamespace("HtmlSharp");
			writer.WriteLineNoTabs();
			...
			using (new CurlyBrackets(writer))
			{
				...
				{
					string propertyName = ConvertToCSharpNaming(attribute);

					writer.WriteLineNoTabs();
					if (propertyName == className)
					{
						writer.WriteLine($"public string? {propertyName}_");
					}
					else
					{
					...
					using (new CurlyBrackets(writer))
					{
						...
						using (new CurlyBrackets(writer))
						{
							...
							writer.WriteLine("WriteValueIfNotNull(value);");
						}
					}
					writer.WriteLineNoTabs();
					writer.WriteLine($"public {className} With{propertyName}()");
					using (new CurlyBrackets(writer))
					{
						writer.WriteLine($"writer.Write(\" {attribute}\");");
					...
					writer.WriteLineNoTabs();
					writer.WriteLine($"public {className} With{propertyName}(string value)");
					using (new CurlyBrackets(writer))
					{
						writer.WriteLine($"writer.Write(\" {attribute}=\\\"\");");
				...
				writer.WriteLineNoTabs();
				writer.WriteLine($"public {className} WithCustomAttribute(string key)");
				using (new CurlyBrackets(writer))
				{
				...
				writer.WriteLineNoTabs();
				writer.WriteLine($"public {className} WithCustomAttribute(string key, string value)");
				using (new CurlyBrackets(writer))
				{
					writer.WriteLine("WriteKey(key);");
				...
				writer.WriteLineNoTabs();
				writer.WriteLine("private void WriteValueIfNotNull(string? value)");
				using (new CurlyBrackets(writer))
				{
					using (new If(writer, "value is not null"))
					{
						writer.WriteLine("WriteValue(value);");
					}
				}
	...
	static string ConvertToCSharpNaming(string kebabCase)
	{
